...
# Mutable in-memory itinerary state
current_itinerary = SAMPLE_ITINERARY_DATA.model_copy(deep=True)
...
async def add_activity(id: str, payload: AddActivityRequest):
    day = next((d for d in current_itinerary.days if d.dayNumber == payload.dayNumber), None)
    if not day:
    ...
    act_obj = ActivityItem(
        ...
        location=new_act_data.get("location", "Udaipur, Rajasthan"),
        ...
        status="Suggested",
        image=new_act_data.get("image", "https://images.unsplash.com/photo-1599661046289-e31897846e41?auto=format&fit=crop&w=800&q=80"),
        votes=ActivityVotes(up=1, down=0)
    )

    day.activities.append(act_obj)
    current_itinerary.budget.spent += act_obj.cost

    return {"message": "Activity added successfully", "itinerary": current_itinerary}
...
async def remove_activity(id: str, activity_id: str):
    ...
    return {"message": "Activity removed", "itinerary": current_itinerary}
